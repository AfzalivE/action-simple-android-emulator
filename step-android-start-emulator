...
- bash: |
    ...
    echo 'y' | $ANDROID_HOME/tools/bin/sdkmanager --install "system-images;android-${{ parameters.sdk }};google_apis;x86_64"
    echo 'no' | $ANDROID_HOME/tools/bin/avdmanager create avd -n "emulator-${{ parameters.sdk }}" -d "${{ parameters.device }}" -k "system-images;android-${{ parameters.sdk }};google_apis;x86" -f
